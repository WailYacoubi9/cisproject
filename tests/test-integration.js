...
${colors.reset}\n`);
  ...
  try {
    const response = await axios.get(`${DEVICE_URL}/health`, {
      timeout: 5000
    });
    if (response.data.status === 'OK') {
      logSuccess('Device-app est accessible (GET /health)');
    } else {
      logError('Device-app répond mais status incorrect');
    }
  } catch (error) {
    logError('Device-app non accessible', error);
  }

  // Test 4: Device peut demander un code
  logSection('TEST 2: Device Flow - Demande de code');

  try {
    const response = await axios.post(`${DEVICE_URL}/start-device-flow`, {}, {
      headers: { 'Content-Type': 'application/json' },
      timeout: 10000
    });

    if (response.data.success && response.data.data.user_code) {
      logSuccess(`Device peut demander un code (user_code: ${response.data.data.user_code})`);
      logInfo(`   Code: ${response.data.data.user_code}`);
      logInfo(`   Expire dans: ${response.data.data.expires_in}s`);
    } else {
      logError('Device Flow échoue - format de réponse incorrect');
    }
  } catch (error) {
    logError('Device Flow échoue', error);
  }

  // Test 5: /api/status retourne 404 (vérification architecture)
  logSection('TEST 3: Architecture - Routes supprimées');

  try {
    await axios.get(`${DEVICE_URL}/api/status`, {
      timeout: 5000
    });
    logError('/api/status existe encore (devrait être supprimé)');
  } catch (error) {
    if (error.response && error.response.status === 404) {
      logSuccess('/api/status retourne 404 (correctement supprimé)');
    } else {
      logError('/api/status - erreur inattendue', error);
    }
  }
